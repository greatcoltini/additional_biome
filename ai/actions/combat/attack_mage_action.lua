...
local AttackMage = class()

AttackMage.name = 'attack mage in sight'
AttackMage.does = 'additional_biomes:combat:attack_mage'
AttackMage.args = {
   target = Entity
}
AttackMage.version = 2
AttackMage.priority = 1
AttackMage.weight = 1

-- TODO: prohibit attacking at melee range
function AttackMage:start_thinking(ai, entity, args)	
   ...
   self._attack_types = stonehearth.combat:get_combat_actions(entity, 'additional_biomes:combat:mage_attacks')
...
function AttackMage:_choose_attack_action(ai, entity, args)
   ...
   self._think_timer = stonehearth.combat:set_timer("AttackMage waiting for cooldown", 1000, function()
         self._think_timer = nil
         self:_choose_attack_action(ai, entity, args)
      end)
end

function AttackMage:stop_thinking(ai, entity, args)
...
function AttackMage:run(ai, entity, args)
...
function AttackMage:_add_shoot_timer(entity, target, time_to_shoot)
   local shoot_timer = stonehearth.combat:set_timer("AttackMage shoot", time_to_shoot, function()
      self:_shoot(entity, target, self._weapon_data)
   end)
   table.insert(self._shoot_timers, shoot_timer)
end

function AttackMage:stop(ai, entity, args)
...
function AttackMage:_shoot(attacker, target, weapon_data)
   ...
   local projectile = self:_create_projectile(attacker, target, projectile_speed)
...
function AttackMage:_create_projectile(attacker, target, projectile_speed)
   local projectile = radiant.entities.create_entity('additional_biomes:weapons:fireball')
   local projectile_component = projectile:add_component('stonehearth:projectile')
...
function AttackMage:_get_world_location(point, entity)
